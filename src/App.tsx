...
import { submitEncounterEvaluation, startSimulationSession } from './services/apiClient';

const MainContent: React.FC = () => {
  // Navigation View
  const [currentView, setCurrentView] = useState<'landing' | 'dashboard' | 'library' | 'simulation' | 'evaluation' | 'admin' | 'educator'>('landing');
  ...
  const handleStartCase = async (c: ClinicalCase) => {
    ...
    const sessionId = await startSimulationSession(c.id);

...
import { checkBackendStatus, sendPatientChatMessage, BackendStatus } from '../services/apiClient';
...
export const SimulationRoom: React.FC<SimulationRoomProps> = ({
  ...
  const handleSendMessage = async (textToSend?: string) => {
    ...
    try {
      const responseData = await sendPatientChatMessage(clinicalCase, text, newHistory, sessionId);
      ...
      const fallbackMessage: ChatMessage = {
        id: `msg-${Date.now()}-pt`,
        sender: 'patient',
        text: "I'm feeling so uncomfortable... Can you help me, doctor?",
        timestamp: new Date().toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' }),
        category: 'General'
      };
  ...
  const handlePerformExam = (exam: PhysicalFinding) => {
    if (!performedExamIds.includes(exam.id)) {
      setPerformedExamIds(prev => [...prev, exam.id]);
      
      const examMsg: ChatMessage = {
        id: `msg-${Date.now()}-sys`,
        sender: 'system',
        text: `[Physical Examination] Performed ${exam.name}: ${exam.findingDescription}`,
        timestamp: new Date().toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' }),
        category: 'Exam'
      };
  ...
  // Order Investigation
  const handleOrderInvestigation = (inv: InvestigationResult) => {
  ...
  return (
    <div className="flex flex-col h-[calc(100vh-4rem)] bg-[#F7F4EE] text-[#1A2928] overflow-hidden">
        ...
        <div className={`lg:col-span-5 flex flex-col bg-[#F7F4EE] overflow-hidden ${
          ...
          {activeTab === 'exam' && (
            <div className="flex-1 p-3.5 sm:p-5 overflow-y-auto space-y-3 sm:space-y-4">
              ...
              <div className="space-y-3">
                {clinicalCase.physicalFindings.map((exam) => {
          ...
          {activeTab === 'investigations' && (
            <div className="flex-1 p-3.5 sm:p-5 overflow-y-auto space-y-3 sm:space-y-4">
              ...
              <div className="space-y-3">
                {clinicalCase.investigations.map((inv) => {

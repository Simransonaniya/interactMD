...
import { fetchCases, getAuthHeaders } from '../services/apiClient';
...
export const AdminCMS: React.FC = () => {
  ...
  const handleCreateCase = async (e: React.FormEvent) => {
    ...
    try {
      const res = await fetch('http://localhost:8000/api/v1/admin/cases', {
        method: 'POST',
      ...
      if (!res.ok) {
        ...
        throw new Error(err.detail || 'Failed to create case in PostgreSQL.');
      }

      setStatusMessage(`Case '${title}' successfully created in PostgreSQL database!`);
    ...
    <div className="max-w-[1700px] mx-auto px-4 sm:px-8 lg:px-12 py-10 space-y-9">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-5">
        <div>
          ...
          <p className="mt-2 text-sm sm:text-base text-[#1A2928]/70 max-w-2xl font-normal">
            Author, clinically review, simulate test runs, and publish dynamic clinical encounter scenarios directly into PostgreSQL.
          </p>
        </div>
      ...
      {isCreatingDraft && (
        <form onSubmit={handleCreateCase} className="bg-white rounded-3xl border border-[#39605B]/40 p-7 sm:p-8 shadow-xl space-y-5 animate-slide-up">
          ...
          <div className="flex justify-end space-x-3 pt-3">
            ...
            >
              {isSubmitting ? 'Saving to Database...' : 'Save & Publish Case to PostgreSQL'}
            </button>
          </div>
        </form>
      )}
      ...
      <div className="bg-white rounded-3xl border border-[#39605B]/15 shadow-xs overflow-hidden">
        <div className="p-6 border-b border-[#39605B]/15 flex items-center justify-between">
          ...
          <span className="text-xs text-[#1A2928]/60">PostgreSQL Live Store</span>
        </div>
